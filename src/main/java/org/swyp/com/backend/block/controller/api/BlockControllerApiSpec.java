...
public interface BlockControllerApiSpec {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **동작 방식**
                    ...
                    - 차단 즉시 나(차단자)와 상대방(피차단자) 양쪽 모두의 보관함 목록(`GET /exchange/archive`)에서 서로가 보이지 않게 됩니다. 실제 보관함 데이터는 삭제되지 않고 조회에서만 제외되므로, 차단 해제 시 원래대로 다시 보입니다.
                    - 피차단자에게는 차단 사실이 어떤 방식으로도 노출되지 않습니다. 자신의 보관함 목록에서 상대방이 조용히 사라지는 것과 동일하게 보입니다.
    ...
    @Operation(
            summary = "차단 해제",
            description = """
                    차단을 해제합니다. 차단 시 조회에서만 제외되고 실제 보관함 데이터는 삭제되지 않았으므로, \
                    해제 즉시 서로의 보관함 목록에 원래 항목이 그대로 다시 노출됩니다.
                    """,
